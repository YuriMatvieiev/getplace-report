function initPageElements() {
  ...
  const initSection = function (sectionId, buttonClass, plan) {
    ...
    let backButtons = section.getElementsByClassName(
      "confirm__form-back-button"
    );
    ...
    const locationAddressTitle = document.getElementById(
        ...
        if (errors.length === 0) {
          ...
          xhttp.onreadystatechange = () => {
            if (xhttp.readyState === 4)
              if (xhttp.status === 200)
                try {
                  ...
                  if (resp?.status === "ok") {
                    ...
                    if (selectedPlan === 0)
                      window.location.href = "https://getplace.io/";
              ...
              else sErrs.ShowError("Unexpected error");
          };
...
if (document.getElementById("full-sub-section")) {
  initPageElements();
}
